...
public class MissingPatientProblem {
    ...
    public static int getMissingPatient(int[] array, int low, int high) {
        ...
        if (avg[0] < 0)  // avg is not in the array so avg is missing patient
            return avg[1];

        swap(array, low, avg[0]);  // pivot that avg value assign to the first element

        int pivot = partition(array, low, high);

        // T(n)=T(n/2)+Θ(n).
        if (array[avg[1] - 1] == pivot) // missing patient on the right side of the pivot
            return getMissingPatient(array, avg[1], high);
        else // missing patient on the left side of the pivot
            return getMissingPatient(array, 0, avg[1] - 2);
    ...
    public static int[] getAverage (int[] array, int low, int high) {
        ...
        if (avgIndex == -1)
            return new int[]{-1,average};   // Missing patient number is average value So return -1 index
        else
            return new int[]{avgIndex,average};

    }

    // get index of value in array between low and high indexes
    public static int getIndex(int[] array, int value, int low, int high) {
